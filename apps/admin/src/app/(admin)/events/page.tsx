...
const FILTERS: Array<[FilterKey, string]> = [['all', 'All'], ['live', 'Live'], ['draft', 'Drafts'], ['past', 'Past']];
      ...
      <PaneHeader
        ...
        cta={
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            }}>Duplicate last edition</button>
            <button style={{
              padding: '9px 14px', borderRadius: 8, background: 'var(--hof-amber)',
      ...
      <div style={{ padding: '14px 28px 28px' }}>
        <div style={{
          ...
          <div style={{
            ...
            <div>Ed</div><div>Name</div><div>Date</div><div>Status</div><div>Sold</div><div>Gross</div><div />
          </div>
          {filtered.map((e, i) => (
            <div key={e.ed} style={{
              display: 'grid', gridTemplateColumns: '60px 2fr 1fr 0.7fr 1.2fr 1fr 80px',
              ...
              <div style={{
                color: 'var(--hof-text-sec)', fontFamily: 'JetBrains Mono, monospace', fontSize: 12,
              }}>{e.date}</div>
              <div>
                <Pill
                  tone={e.status === 'live' ? 'amber' : e.status === 'draft' ? 'neutral' : 'success'}
                  size="sm"
                >{e.status}</Pill>
              </div>
              <div style={{ minWidth: 0 }}>
                <div style={{
                  fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: 'var(--hof-text)',
                  fontVariantNumeric: 'tabular-nums',
                }}>{e.sold} / {e.cap}</div>
                <div style={{ height: 4, background: 'var(--hof-elevated)', borderRadius: 2, marginTop: 4 }}>
                  <div style={{
                    height: '100%',
              ...
              <div style={{
                fontFamily: 'JetBrains Mono, monospace', fontSize: 13, color: 'var(--hof-text)',
                fontVariantNumeric: 'tabular-nums',
              }}>{e.gross}</div>
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path stroke="var(--hof-text-sec)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" d="M9 6 L15 12 L9 18"/>
                </svg>
              </div>
            </div>
          ))}
