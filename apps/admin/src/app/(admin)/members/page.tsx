    ...
    <>
      <div style={{
        ...
        <div style={{
          ...
          padding: '7px 12px', background: 'var(--hof-surface)', border: '1px solid var(--hof-border)',
      ...
      <div style={{ padding: '20px 28px 28px' }}>
        <div style={{
          ...
          <div style={{
            ...
            <div>Member</div><div>Email</div><div>Tier</div><div>Editions</div><div>Last seen</div><div>Posts</div><div />
          </div>
          {filtered.map((m, i) => {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      {m.role === 'crew' && <Pill tone="crew" size="sm">Crew</Pill>}
                      {m.flag === 'flagged' && <Pill tone="danger" size="sm">Flagged</Pill>}
                      {m.flag === 'new' && <Pill tone="amber" size="sm">New</Pill>}
                      {m.flag === 'photographer' && <Pill tone="gold" size="sm">Photo</Pill>}
                    </div>
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'var(--hof-text-dis)' }}>
                      Joined {m.joined}
                    </div>
                  </div>
                </div>
                <div style={{
                  color: 'var(--hof-text-sec)', fontSize: 12,
                  overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                }}>{m.email}</div>
                <div>
                  <Pill tone={m.tier === 'VIP' || m.tier === 'Owner' ? 'gold' : 'neutral'} size="sm">
                    {m.tier}
                  </Pill>
                </div>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>
                  {m.editions}
                </div>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: 'var(--hof-text-sec)' }}>
                  {m.lastSeen}
                </div>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>
                  {m.posts}
