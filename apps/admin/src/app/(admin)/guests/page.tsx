    ...
    <>
      <div style={{
        ...
        <div>
          ...
          }}>{loading ? 'Loading…' : `${guests.length} confirmed`}</div>
      ...
      <div style={{ padding: '20px 28px 28px' }}>
        ...
        <div style={{
          ...
          <div style={{
            display: 'grid', gridTemplateColumns: '2fr 1.2fr 0.7fr 0.7fr 0.8fr',
          ...
          {!loading && filtered.map((g, i) => {
            ...
            return (
              <div key={g.id} style={{
                ...
                <div><Pill tone={isVip ? 'gold' : 'neutral'} size="sm">{g.tier}</Pill></div>
                <div><Pill tone={statusTone(g.status)} size="sm">{g.status}</Pill></div>
                <div style={{
                  color: 'var(--hof-text-sec)', fontFamily: 'JetBrains Mono, monospace', fontSize: 11,
                }}>{g.purchased}</div>
              </div>
            );
          })}
          {!loading && filtered.length === 0 && (
            <div style={{
              padding: '24px 18px', fontFamily: 'Inter, system-ui', fontSize: 13,
              color: 'var(--hof-text-sec)', textAlign: 'center',
            }}>No guests match your search.</div>
          )}
        </div>
      </div>
    </>
