...
export default function FinancialsPage() {
  ...
  const maxGross = editionBars.length > 0
    ? Math.max(...editionBars.map((b) => b.gross), 1000)
    : 8000;
  ...
  const activeEditionLabel = activeEdition ? `Edition ${activeEdition.edition_number}` : 'Latest edition';
    ...
    <>
      ...
      <div style={{ padding: '20px 28px 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          <Kpi label={`${activeEditionLabel} gross`} value={loading ? '…' : activeGross} delta="Current event" tone="amber" />
          <Kpi label="Ticket count" value={loading ? '…' : String(activeEdition?.ticket_count ?? 0)} delta="Valid + used tickets" tone="neutral" />
          <Kpi label="Total gross all editions" value={loading ? '…' : totalGross} delta="All time" tone="amber" />
      ...
      <div style={{
        ...
        <div style={{
          ...
          }}>{loading ? 'Loading…' : 'Steady right-up-and-to-the-right'}</div>
          ...
          <div style={{
            ...
            {!loading && editionBars.map((b) => {
              const heightPct = (b.gross / maxGross) * 100;
              return (
                <div key={b.edition} style={{
        ...
        <div style={{
            ...
            return (
              <div key={f.event_id} style={{ marginBottom: 12 }}>
                <div style={{
                  ...
                  <span style={{
                    fontFamily: 'JetBrains Mono, monospace', color: 'var(--hof-text)',
                    fontVariantNumeric: 'tabular-nums',
                  }}>
                    {value}
                  </span>
                </div>
                <div style={{ marginTop: 5, height: 5, background: 'var(--hof-elevated)', borderRadius: 3 }}>
                  <div style={{
                    height: '100%',
                    width: `${pct * 1.7}%`,
                    background: color,
          ...
          <div style={{
            marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--hof-border)',
            display: 'flex', justifyContent: 'space-between',
            fontFamily: 'Inter, system-ui', fontSize: 14,
            color: 'var(--hof-text)', fontWeight: 500,
          }}>
            <span>Total gross</span>
            <span style={{
              fontFamily: 'Clash Display, system-ui', fontWeight: 600, fontSize: 18,
              color: 'var(--hof-amber)', fontVariantNumeric: 'tabular-nums',
            }}>{loading ? '…' : totalGross}</span>
          </div>
        </div>
      </div>
