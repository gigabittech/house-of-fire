...
function ReportCard({ r }: { r: Report }) {
  return (
    <div style={{
      padding: 14, background: 'var(--hof-bg)', border: '1px solid var(--hof-border)', borderRadius: 10,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        <Pill tone={r.severity === 'high' ? 'danger' : 'warning'} size="sm">{r.kind}</Pill>
        <span style={{ fontFamily: 'Inter, system-ui', fontSize: 11, color: 'var(--hof-text-sec)' }}>
          Reported by {r.reporter} · {r.age} ago
        </span>
      </div>
      <div style={{
        padding: '10px 12px', background: 'var(--hof-elevated)', borderRadius: 8,
      ...
      <div style={{
        display: 'flex', gap: 6, marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--hof-border)',
      }}>
        <button style={{
        ...
        <button style={{
          padding: '6px 12px', borderRadius: 6, background: 'transparent',
          border: '1px solid var(--hof-border)', cursor: 'pointer',
          fontFamily: 'Inter, system-ui', fontSize: 12, color: 'var(--hof-text)',
        }}>Hide post</button>
        <button style={{
          padding: '6px 12px', borderRadius: 6, background: 'rgba(232,74,26,0.12)',
          border: '1px solid rgba(232,74,26,0.3)', cursor: 'pointer',
          fontFamily: 'Inter, system-ui', fontSize: 12, color: 'var(--hof-error)',
        }}>Ban author</button>
      </div>
    </div>
  );
...
function QueueCard({ q, onDelete }: { q: QueueItem; onDelete: (id: string) => void }) {
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        <Pill tone="amber" size="sm">{q.kind}</Pill>
        <span style={{
          fontFamily: 'JetBrains Mono, monospace', fontSize: 10,
          color: 'var(--hof-amber)', letterSpacing: '0.04em',
        }}>{q.channel}</span>
        <span style={{ fontFamily: 'Inter, system-ui', fontSize: 11, color: 'var(--hof-text-sec)' }}>· {q.age} ago</span>
      </div>
      ...
      <div style={{
        ...
        <button style={{
          padding: '6px 12px', borderRadius: 6, background: 'var(--hof-amber)', border: 'none', cursor: 'pointer',
  ...
  return (
    <>
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={{
            ...
            border: '1px solid var(--hof-border)', cursor: 'pointer',
            fontFamily: 'Inter, system-ui', fontSize: 13, fontWeight: 500, color: 'var(--hof-text)',
          }}>Mod log</button>
          <button style={{
            padding: '9px 14px', borderRadius: 8, background: 'var(--hof-amber)', border: 'none', cursor: 'pointer',
            fontFamily: 'Inter, system-ui', fontSize: 13, fontWeight: 600, color: 'var(--hof-bg)',
          }}>Promote to Crew</button>
        </div>
      </div>
      ...
      <div style={{
        ...
        {/* Reports queue */}
        <div style={{
          background: 'var(--hof-surface)', border: '1px solid var(--hof-border)', borderRadius: 12, padding: 18,
        }}>
          <div style={{
            ...
            <Pill tone="danger" size="sm">{REPORTS.length} pending</Pill>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <Pill tone="amber" size="sm">{loading ? '…' : `${queue.length} waiting`}</Pill>
          </div>
          ...
          {loading && (
            <div style={{
              padding: '16px', fontFamily: 'Inter, system-ui', fontSize: 13,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path stroke="var(--hof-amber)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
