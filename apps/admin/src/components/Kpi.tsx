  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: 'var(--hof-text)', marginTop: 6, letterSpacing: '-0.01em',
        fontVariantNumeric: 'tabular-nums',
      }}>{value}</div>
      {delta && (
        <div style={{
          fontFamily: 'Inter, system-ui', fontSize: 11, color: deltaColor, marginTop: 4,
        }}>{delta}</div>
      )}
      {progress !== undefined && (
        <div style={{
          marginTop: 10, height: 4, background: 'var(--hof-elevated)', borderRadius: 2,
        }}>
          <div style={{
            height: '100%', width: `${progress}%`,
            background: 'var(--hof-amber)', borderRadius: 2,
          }} />
        </div>
      )}
    </div>
