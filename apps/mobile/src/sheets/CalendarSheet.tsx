...
export default function CalendarSheet({ open, onClose }: CalendarSheetProps) {
  ...
  const icsContent = [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//House of Fire//EN',
    'BEGIN:VEVENT',
    `UID:hof-${dStart}@houseoffire.events`,
    `DTSTAMP:${fmtUTC(new Date())}`,
    `DTSTART:${dStart}`, `DTEND:${dEnd}`,
    `SUMMARY:${EV.title}`, `LOCATION:${EV.location}`, `DESCRIPTION:${EV.details}`,
    'END:VEVENT', 'END:VCALENDAR',
  ].join('\n');
  const icsHref = `data:text/calendar;charset=utf-8,${encodeURIComponent(icsContent)}`;

  type Option = { id: string; label: string; sub: string; icon: string; href: string; target?: string; download?: string };
          ...
          {options.map((o) => (
            ...
            >
              <div style={{
                ...
                <Icon name={o.icon as Parameters<typeof Icon>[0]['name']} size={18} color={colors.text} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: 14, color: colors.text }}>
                  {o.label}
                </div>
                <div style={{ fontFamily: 'Inter', fontSize: 12, color: colors.textSec, marginTop: 1 }}>
                  {o.sub}
                </div>
              </div>
