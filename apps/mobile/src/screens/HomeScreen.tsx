...
export default function HomeScreen() {
  ...
  useEffect(() => {
    fetch('/api/events/upcoming')
      .then(r => r.json())
      .then(d => { if (d.event) setUpcomingEvent(d.event); })
      .catch(console.error);
  }, []);
  ...
  const eventTs = Date.parse(
    upcomingEvent?.date
      ? `${upcomingEvent.date}T20:00:00-06:00`
      : '2026-06-26T20:00:00-06:00'
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          <GhostBtn onClick={() => router.push('/event')}>
            Event details
          </GhostBtn>
        </div>
        ...
        <div style={{ padding: '28px 16px 8px' }}>
          ...
          >
            A monthly gathering of the people who keep coming back. Underground
            house and techno. One room. One night a month. Always at the
            Junkyard.
          </div>
        </div>
        ...
        >
          ...
          >
            One short message before each edition. Lineup, doors, dress code.
            Nothing else.
          </div>
          {newsSent ? (
            <div
